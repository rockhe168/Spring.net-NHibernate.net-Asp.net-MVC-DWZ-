...
namespace Dao.NHibernateImpl.SysManager
{
    public class DepartmentDao : NHibernateRepository<Department>, IDepartmentDao
    {
        ...
        //    try
        //    {
        //        string hql = string.Format("select distinct a.*,b.Name as CompanyName from Department a left join Company b on a.Company=b.CompanyId {0} order by {1}", 
        ...
        //                session =>
        //                session.CreateSQLQuery(hql).AddEntity("a",typeof(Department)).AddScalar("CompanyName",).SetFirstResult((currentPageNo - 1) * pageSize).SetMaxResults(pageSize).
        //                    List<Department>());
        //        return list;
        //    }
        //    catch (Exception ex)
