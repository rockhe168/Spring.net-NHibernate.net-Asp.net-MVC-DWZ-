...
function dwzPageBreak(options){
	...
	if (op.rel) {
		...
		/*****Start Rock 2012.9.7号加入，防止点击分页清空查询条件*****/
		var pageHeaderForm = $("#pageHeader", $parent).get(0);
		var pageHeaderParams = $(pageHeaderForm).serializeArray();

		var paramsLength = params.length;
		for (var i = 0; i < pageHeaderParams.length; i++) {
		    params[paramsLength + i] = pageHeaderParams[i];
		}
		/*****End Rock 2012.9.7号加入，防止点击分页清空查询条件*****/
